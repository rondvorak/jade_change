...
var parallaxPage = function(name, htmlObject){
	return{
      ...
      console.log('WTF?!?!@?!?')
		  //catach parallax before it tries to change pages
      if(typeof parallax.preload === 'function'){ parallax.preload(this.key); }
			if(typeof this.preload === 'function'){ this.preload(this.key); }
			if(!parallax.sliding){
      ...
      css.useTranslate3d = true
		...
		show : function(newLocation){
			...
			this.page.css(newLocation);
      // set the hash here
      //window.location.hash = '_' + this.key;
			return this;
		},
